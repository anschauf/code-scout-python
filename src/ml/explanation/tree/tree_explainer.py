...
from src.ml.explanation.tree.utilities.lists import true_list
from src.ml.explanation.tree.utilities.model_validation import validate_model_is_trained, validate_model_type
from src.ml.explanation.tree.utilities.numerical import divide0
from src.ml.explanation.tree.utilities.parallel import analyze_tree_structure, compute_explanation_of_prediction, \
    compute_feature_contributions_from_tree, compute_two_way_conditional_contributions
# Local repo
from src.ml.explanation.tree.utilities.preprocessing import DataProcessor
from src.ml.explanation.tree.utilities.visualization import adjust_spines
...
class TreeExplainer(object):
    def __init__(self, model, data, targets=None, n_jobs=None, verbose=False):
        ...
        :param verbose: [bool or int] Whether to logger.debug progress to the console.
        ...
        if self.verbosity_level > 0:
            logger.debug('Analyzing model structure and computing feature contributions ...')
    ...
    def explain_feature_contributions(self, joint_contributions=False,
        ...
        # Process trees in parallel
        if self.joblib_params['n_jobs'] > 1:
            ...
            trees = getattr(self.model, self._model_specifics['estimators_'])
            for i_tree, estimator in tqdm(enumerate(trees), total=len(trees)):
    ...
    def explain_single_prediction(self, observation_idx, solve_duplicate_splits='mean',
        ...
        with pd.option_context('display.max_rows', None, 'display.max_columns', None):
            ...
            if decision_table_numerical.shape[0] > 0:
                logger.debug(decision_table_numerical)
            if decision_table_categorical.shape[0] > 0:
                logger.debug(decision_table_categorical)
    ...
    def compute_two_way_interactions(self, n_jobs=-1, verbose=False):
        ...
        :param verbose: [bool] Whether to logger.debug progress messages.
        ...
        for i_iter in range(n_iterations):
            ...
            if verbose:
                logger.debug('Computing two-way feature interactions of %s predictions ...' % key_name)
