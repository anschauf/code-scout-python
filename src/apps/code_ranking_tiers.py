    ...
    for _, method_name, ranked_cases in all_rankings:
        ...
        for _, current_revised_case in revised_cases.iterrows():
            ...
            if ranked_suggestions.shape[0] == 0:
                ...
                logger.warning(f'The case ID {case_id} did not have any suggestions')
                continue
            ...
            chop_added_list = current_revised_case['CHOP_added_split']
        ...
        code_ranks.append(pd.DataFrame(np.vstack(current_method_code_ranks), columns=[case_id_col, 'added_ICD', 'ICD_suggested_list', 'rank']))
