...
def main(dir_output):
    ...
    X_train_pccl, pccl_label, encoder_pccl = categorize_variable(data_train, 'pccl')
    X_test_pccl, _, _ = categorize_variable(data_test, 'pccl', encoder=encoder_pccl)

    X_train_cw = data_train['effective_cost_weight']
    X_test_cw = data_test['effective_cost_weight']

    X_train_dos = data_train['duration_of_stay']
    X_test_dos = data_test['duration_of_stay']

    # create number of code fields
    X_train_number_of_sdx = data_train['code_diagnoses'].apply(lambda x: len(x) - 1)
    X_test_number_of_sdx = data_test['code_diagnoses'].apply(lambda x: len(x) - 1)

    X_train_number_of_chops = data_train['code_procedures'].apply(lambda x: len(x))
    X_test_number_of_chops = data_test['code_procedures'].apply(lambda x: len(x))

    # X_train_number_of_used_diags = data_train['code_procedures'].apply(lambda x: len(x))
    ...
    predictor_labels = np.concatenate([gender_label, pccl_label, 'effective_cost_weight', 'duration_of_stay', 'number_of_sdx', 'number_of_chops'])
    y_label = 'y_label_is_revised_case'
    X_train = np.hstack([X_train_gender, X_train_pccl, X_train_cw, X_train_dos, X_train_number_of_sdx, X_train_number_of_chops])
    y_train = data_train[y_label]
    X_test = np.hstack([X_test_gender, X_test_pccl, X_test_cw, X_test_dos, X_test_number_of_sdx, X_test_number_of_chops])
