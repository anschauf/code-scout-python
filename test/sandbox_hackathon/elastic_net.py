...
def main(dir_output):
    ...
    # train the model
    model = train_lr_model(X_train, y_train, penalty='elasticnet', class_weight='balanced', solver='saga', random_state=RANDOM_SEED, fit_intercept=False)

    # evaluate model how well it predicts
    # write model coefficients to file
    write_model_coefs_to_file(model, join(dir_output, 'model_coefs_elasticnet.csv'), predictor_labels=predictor_labels)
    ...
    write_evaluation_metrics_to_file(y_train, prediction_probas_train, y_test, prediction_probas_test, join(dir_output, 'metrics_random_forest.csv'), threshold=0.5)

    # write performance app output to file for case ranking comparisons
    extract_case_ranking_performance_app(data_test, prediction_probas_test, join(dir_output, 'performance_app_input_elasticnet.csv'))
