...
RESULTS_DIR = join(ROOT_DIR, 'results', f'random_forest_only_reviewed_k-fold_{LEAVE_ON_OUT[0]}_{LEAVE_ON_OUT[1]}', f'n_trees_{RANDOM_FOREST_NUM_TREES}-max_depth_{RANDOM_FOREST_MAX_DEPTH}-min_samples_leaf_{RANDOM_FOREST_MIN_SAMPLES_LEAF}')
...
def train_random_forest_only_reviewed_cases():
    ...
    with warnings.catch_warnings():
        ...
        ind_train_val = np.concatenate([ind_train, ind_test])
        for train_indices, test_indices in tqdm(cv.split(features[ind_train_val], y[ind_train_val]), total=5):
