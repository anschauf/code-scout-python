...
def train_xg_boost_only_reviewed_cases():
    for NUM_TREES in [100]:
        for MAX_DEPTH in [2, 3, 4, 5, 10]:
            # for LEARNING_RATE in [0.5, 0.7, 0.9, 1.0, 1.1, 1.3, 1.5, 2.0]:
            for MIN_CHILD_WEIGHT in [10, 20, 50, 100, 150, 200, 300, 500, 1000]:
                ...
                with warnings.catch_warnings():
                    ...
                    estimator = XGBClassifier(
                        ...
                        seed=RANDOM_SEED,
                        min_child_weight=MIN_CHILD_WEIGHT # should be also adapted and tried out / range: 0 - infinity
                    )
                ...
                with open(join(RESULTS_DIR, 'rf_cv.pkl'), 'wb') as f:
                    ...
                    pd.DataFrame({
                        ...
                        join(RESULTS_DIR, 'feature_importances_xg_boost.csv'), index=False)
...
if __name__ == '__main__':
    train_xg_boost_only_reviewed_cases()
    sys.exit(0)
