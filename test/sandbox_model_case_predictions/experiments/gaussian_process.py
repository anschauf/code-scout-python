...
def train_random_forest_only_reviewed_cases():
    ...
    for LEAVE_ON_OUT in [
        ...
        folder_name = f'test_GP_{LEAVE_ON_OUT[0]}_{LEAVE_ON_OUT[1]}'
        ...
        if not os.path.exists(RESULTS_DIR):
            ...
            with warnings.catch_warnings():
                ...
                kernel = 1.0 * RBF(1.0)
                estimator = GaussianProcessClassifier(kernel=kernel, random_state=RANDOM_SEED, n_jobs=-1)
                estimator.fit(features[ind_train], y[ind_train])
            ...
            for info in tqdm(all_hospitals_and_years):
                ...
                if hospital_name == LEAVE_ON_OUT[0] and discharge_year == LEAVE_ON_OUT[1]:
                    ...
                    predictions = estimator.predict_proba(test_features)[:, 1]
