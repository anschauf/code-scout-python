...
FOLDER_NAME = f'lr_baseline_use_ICA_reduced_features_{LEAVE_ON_OUT[0]}_{LEAVE_ON_OUT[1]}'
RESULTS_DIR = join(ROOT_DIR, 'results', FOLDER_NAME,
                   f'{n_components}_components_{hyperparameter_info}')
...
def feature_reduction_only_reviewed_cases():
    ...
    # preprocessing features (scaler) and applying ICA
    ...
    for info in tqdm(all_hospitals_and_years):
        ...
        if hospital_name == LEAVE_ON_OUT[0] and discharge_year == LEAVE_ON_OUT[1]:
            ...
            data_rescaled_test = scaler.transform(test_features)
            feature_test_from_pca = ica_transformer.transform(data_rescaled_test)
            ...
            filename_output = join(RESULTS_DIR_TEST_PREDICTIONS,
                                   f'{FOLDER_NAME}_{n_components}_components_{hyperparameter_info}.csv')
