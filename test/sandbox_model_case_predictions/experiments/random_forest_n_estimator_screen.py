...
dir_output = join(ROOT_DIR, 'results', f'random_forest_n_estimator_screen_{hospital_year_for_performance_app[0]}_{hospital_year_for_performance_app[1]}')
...
def train_test_random_forest():
    ...
    with warnings.catch_warnings():
        ...
        for n_estimator in np.arange(1, 10, 1):
            logger.info(f'Training with {n_estimator=} ...')
            # train model
            ...
            with open(join(dir_output, f'rf_5000_n_estimator_{n_estimator}.pkl'), 'wb') as f:
            ...
            pd.DataFrame({
                ...
                join(dir_output, f'feature_importances_random_forest_n_estimator_{n_estimator}.csv'), index=False)
            ...
            create_predictions_output_performance_app(filename=join(dir_output, f'n_estimator_{n_estimator}-{hospital_year_for_performance_app[0]}_{hospital_year_for_performance_app[1]}.csv'),
