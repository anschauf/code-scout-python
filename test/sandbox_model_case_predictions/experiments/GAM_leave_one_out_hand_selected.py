...
if USE_HAND_SELECTED_FEATURES:
    FOLDER_NAME = f'GAM_hand_selected_{LEAVE_ON_OUT[0]}_{LEAVE_ON_OUT[1]}'
...
def LogisticGAM_hand_selected_features_only_reviewed_cases():
    ...
    reviewed_cases_check = reviewed_cases.iloc[ind_all]
    df = reviewed_cases_check[
        (reviewed_cases_check['hospital'] == 'KSW') & (reviewed_cases_check['dischargeYear'] == 2020)]
    ...
    with warnings.catch_warnings():
        ...
        # Run LogisticGAM with different number of Splines
        for splines in [10]:

            hyper_info = f'{splines=}'

            #lam = np.logspace(-5,5,20)
            #lams = lam*len(HAND_SELECTED_FEATURES)

            LogGAM = LogisticGAM(n_splines=splines, penalties='auto', max_iter=10000).gridsearch(features_train, y_train, objective='auto',
                                                                            return_scores=True)
            ...
            metrics.to_csv(join(RESULTS_DIR, f'GAM_predictors_screen_{hyper_info}.csv'))
