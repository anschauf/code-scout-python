...
FOLDER_NAME = f'features_preprocessing_pca'
RESULTS_DIR = join(ROOT_DIR, 'results', FOLDER_NAME,
                   f'{FOLDER_NAME}_{VARIANCE_PERCENT_PCA * 100}%_varaince')
...
def features_preprocessing_pca_only_reviewed_cases():
    ...
    for feature_name in feature_names:
        ...
        feature_id = [f'{feature_name}_{i}' for i in range(feature_values.shape[1])] if feature_values.shape[1] > 1 else [
                feature_name]
        feature_ids.append(feature_id)
        features_all[feature_name] = feature_id


    feature_ids = np.concatenate(feature_ids)
    features = np.hstack(features)
    features_all_df = pd.DataFrame(features_all.items(), columns=['feature_name', 'features'])
    features_all_df.to_csv('features_info.csv')
...
if __name__ == '__main__':
    features_preprocessing_pca_only_reviewed_cases()
    sys.exit(0)
