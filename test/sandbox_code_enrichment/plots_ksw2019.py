...
ground_truth_chops_added = np.unique(np.concatenate([x.split('|') for x in ground_truth['CHOP_added'].values if isinstance(x, str)]))
ground_truth_chops_added = np.asarray([x.split(':')[0].replace('.', '') for x in ground_truth_chops_added])
